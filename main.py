...
if __name__ == '__main__':
    ...
    for dataset_title, dataset_name in datasets.items():
        analysis_results.update({dataset_title: {}})
        ...
        try:
            for k in VALUES_OF_K:
                ...
                if result:
                    ...
                    analysis_results[dataset_title].update({k: (result[0], result[1], result[2], result[3])})
                else:
                    logger.info(f"Failed to analyze dataset '{dataset_title}'")
        except:
            logger.warning(f"Exception while analyzing dataset '{dataset_title}'")
    with open(f"./analysis_data/results.txt", "w+") as f:
        header = "dataset_name"
        for k in VALUES_OF_K:
            header += f";LPCA_{k};MLE_{k};MLENB_{k};TLE_{k}"
        f.write(header+"\n")
        for dataset_title, dataset_results in analysis_results.items():
            row = dataset_title
            for k, (LPCA, MLE, MLENB, TLE) in dataset_results.items():
                row += f";{LPCA};{MLE};{MLENB};{TLE}"
            f.write(row+"\n")
    ...
    for k, dataset_stats_for_k in dataset_stats.items():
        ...
        lidsTLE = []
        skewnesses = []
        f = open(f"./analysis_data/sanity_check_{k}.txt", "w+")
        f.write("dataset_name;skewness;LPCA;MLE;MLENB;TLE\n")
        for dataset_name, ((lidLPCA,lidMLE,lidMLENB,lidTLE), skewness) in dataset_stats_for_k.items():
            ...
            f.write(f"{dataset_name};{skewness};{lidLPCA};{lidMLE};{lidMLENB};{lidTLE}\n")
        f.close()
